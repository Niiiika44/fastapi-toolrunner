...
from app.memory_allocator.tasks import process_test, process_validation, tasks_sweeper
...
def patched_sweeper(mock_uow):
    ...
    service.requeue_stale_validations.return_value = 0
...
async def test_sweep_happy(mock_uow):
    ...
    fake_service = SweeperService(mock_uow, dispatch, dispatch_validation)
    fake_service.requeue_stale_pending = AsyncMock()
    fake_service.requeue_stale_pending.return_value = 3
    fake_service.requeue_stale_validations = AsyncMock()
    fake_service.requeue_stale_validations.return_value = 2
    ...
    assert requeued_num == {"tests": 3, "validations": 2}
    assert service_cls.call_args.args[1] == process_test.delay
    assert service_cls.call_args.args[2] == process_validation.delay


def test_task_returns_service_count(patched_sweeper):
    patched_sweeper.requeue_stale_pending.return_value = 3
    patched_sweeper.requeue_stale_validations.return_value = 2

    assert tasks_sweeper.sweep_stale_jobs() == {"tests": 3, "validations": 2}
...
def test_task_logs_finish_at_info_when_something_requeued(patched_sweeper, caplog):
    ...
    assert finished[0].requeued == {"tests": 2, "validations": 0}
...
def test_task_stays_quiet_at_info_when_nothing_requeued(patched_sweeper, caplog):
    ...
    patched_sweeper.requeue_stale_validations.return_value = 0
