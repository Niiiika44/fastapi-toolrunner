...
async def test_process_test_happy(mock_uow, mock_storage):
    ...
    with patch.object(tasks_testcase, "build_uow", lambda: _fake_uow(mock_uow)), \
         patch.object(tasks_testcase, "get_storage", return_value=mock_storage), \
         patch.object(tasks_testcase, "IngestionService", return_value=fake_service):
        await tasks_testcase._process_test(1)

    assert test.status == tasks_testcase.TestStatus.PROCESSING
...
async def test_process_test_domain_error(mock_uow, mock_storage):
    ...
    with patch.object(tasks_testcase, "get_storage", return_value=mock_storage), \
         patch.object(tasks_testcase, "build_uow", lambda: _fake_uow(mock_uow)), \
         patch.object(tasks_testcase, "IngestionService", return_value=fake_service):
        await tasks_testcase._process_test(1)
